...
import type { CbtQuizQuestion, CbtUserResponse, CbtQuizResultSummary } from "@/types/quiz-cbt";
import {
  CheckCircle,
  XCircle,
  WarningCircle,
...
interface CbtQuizResultsViewProps {
  questions: CbtQuizQuestion[];
  userResponses: Record<number, CbtUserResponse>;
  results: CbtQuizResultSummary;
  examTitle: string;
  onRetake: () => void;
}

export function CbtQuizResultsView({
  questions,
  userResponses,
  results,
  examTitle,
  onRetake
}: CbtQuizResultsViewProps) {
  ...
  const getQuestionResult = (q: CbtQuizQuestion) => {
