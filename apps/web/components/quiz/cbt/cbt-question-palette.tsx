...
import type { CbtQuestionStatus, CbtQuizQuestion, CbtUserResponse } from "@/types/quiz-cbt";
...
export function CbtQuestionPalette({
  ...
  const unitsData = useMemo(() => {
    const map = new Map<string, CbtQuizQuestion[]>();
    questions.forEach((q) => {
  ...
  const counts = useMemo(() => {
    ...
    questions.forEach((q) => {
      ...
      const status: CbtQuestionStatus = resp?.status || "not_visited";
      res[status]++;
    });
  ...
  const getPillClasses = (status: CbtQuestionStatus, isCurrent: boolean) => {
    const base =
