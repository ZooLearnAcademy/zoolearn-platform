...
import type { QuizExamData, UserResponse, QuestionStatus, QuizResultSummary, UnitStat } from "@/types/quiz";
...
export function CBTQuizView({ examData }: CBTQuizViewProps) {
  ...
  const currentStatus: QuestionStatus = currentResponse?.status ?? "not_visited";
  ...
  const resultsSummary: QuizResultSummary = useMemo(() => {
    let correct = 0;
    let incorrect = 0;
    let unattempted = 0;
    ...
    const unitStats: UnitStat[] = Object.entries(unitMap).map(([unit, stats]) => ({
      unit,
      ...stats
    }));
